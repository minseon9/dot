...
const copilotRequest_1 = require("./copilot/copilotRequest");
const structure_tree_manager_1 = require("./explorer/structure-tree-manager");
const explorer_tree_provider_1 = require("./explorer/explorer-tree-provider");
...
function activate(context) {
    ...
    const aiPreferencesConfig = vscode_1.workspace.getConfiguration("boot-java.ai");
    const mcpServerEnabled = aiPreferencesConfig.get("mcp-server-enabled");
    const mcpServerPort = aiPreferencesConfig.get("mcp-server-port", 50627);
    ...
    */
    ...
    return commons.activate(options, context).then(client => {
        context.subscriptions.push(vscode_1.commands.registerCommand('vscode-spring-boot.ls.start', () => client.start().then(() => {
        ...
        context.subscriptions.push(vscode_1.commands.registerCommand('vscode-spring-boot.ls.stop', () => client.stop()));
        ...
        context.subscriptions.push(vscode_1.commands.registerCommand('vscode-spring-boot.agent.apply', guideApply_1.applyLspEdit));
        // Register content loader for URIs of type `spring-boot-ls:...` (load JAR content via Boot LS)
        context.subscriptions.push(vscode_1.workspace.registerTextDocumentContentProvider('spring-boot-ls', new (class {
            provideTextDocumentContent(uri) {
                return vscode_1.commands.executeCommand('sts/jar/fetch-content', uri.toString());
            }
        })()));
        const api = new apiManager_1.ApiManager(client).api;
        // Activation of structure explorer
        const structureManager = new structure_tree_manager_1.StructureManager(context, api);
        new explorer_tree_provider_1.ExplorerTreeProvider(structureManager).createTreeView(context, 'explorer.spring');
        return api;
    });
}
...
function activateSpringBootParticipant(context) {
    return __awaiter(this, void 0, void 0, function* () {
        ...
        if (!models || models.length === 0) {
            copilotRequest_1.logger.error(`No suitable model available. Please make sure you have installed the latest "GitHub Copilot Chat" (v0.16.0 or later) and all \`lm\` API is enabled.`);
            return;
        }
